  ...
  tools { 
   ...
   stages{
	...
	stage('Push') {
            steps {
                script{
                   docker.withRegistry('https://858688858026.dkr.ecr.us-west-2.amazonaws.com', 'ecr:us-west-2:aws-credentials') {
                    app.push("latest")
                    }
                }
            }
	...
	stage('Kubernetes Deployment of ASG Bugg Web Application') {
	   steps {
		  ...
		  sh('kubectl delete all --all -n devsecopss')
		  sh ('kubectl apply -f deployment.yaml --namespace=devsecopss')
		}
	      }
   	}
